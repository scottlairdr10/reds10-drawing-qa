...
export default function App() {
  ...
  const [colView, setColView] = useState('preview')
  ...
  useEffect(() => { chatBottomRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [chatMessages, selectedId])
...
{"summary":"one sentence overview","drawingRef":"ref or Unknown","drawingType":"type identified","overallRAG":"RED|AMBER|GREEN","phases":[{"id":1,"name":"Title Block Completeness","rag":"RED|AMBER|GREEN|NA","naReason":null,"findings":[{"rag":"RED|AMBER|GREEN","finding":"issue description","action":"what to do"}]}],"criticalFindings":[{"phase":"phase name","finding":"description","action":"required action"}],"issueRecommendation":"one line verdict on readiness to issue"}`
    ...
    try {
      const messages = b64
        ...
        : [{ role: 'user', content: `Perform a full QA review of drawing "${drawing.name}". No image — assess from filename only. Return only valid JSON.` }]
      ...
      console.error('analyseDrawing error:', e)
  ...
  const sendChat = async () => {
    ...
    const id = selected.id; const text = chatInput.trim()
    setChatInput('')
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          system: `You are a drawing QA expert for Reds10 Group. Drawing: ${selected.result?.drawingRef || selected.name}. QA result: ${JSON.stringify(selected.result)}. Answer concisely and professionally.`,
          messages: updated,
        }),
      })
  ...
  return (
    <div style={{ display:'flex', height:'100vh', background:'#f7f5f1', overflow:'hidden' }}>
      ...
      <div style={{ width:230, background:BRAND.charcoal, display:'flex', flexDirection:'column', flexShrink:0 }}>
        ...
        {tab==='review' ? (
          <>
            <div onDrop={e=>{e.preventDefault();setDragOver(false);handleFiles(e.dataTransfer.files)}} onDragOver={e=>{e.preventDefault();setDragOver(true)}} onDragLeave={()=>setDragOver(false)} onClick={()=>fileRef.current?.click()} style={{ margin:10, border:`1.5px dashed ${dragOver?BRAND.chalk:'rgba(227,222,210,0.25)'}`, borderRadius:8, padding:'13px 8px', textAlign:'center', cursor:'pointer', background:dragOver?'rgba(255,255,255,0.04)':'transparent' }}>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:'0 8px 8px' }}>
              ...
              {drawings.map(d=>(
                <div key={d.id} onClick={()=>setSelectedId(d.id)} style={{ background:selectedId===d.id?'rgba(222,19,77,0.18)':'rgba(255,255,255,0.05)', border:selectedId===d.id?`1.5px solid ${BRAND.red}`:'1.5px solid transparent', borderRadius:7, padding:'8px 10px', cursor:'pointer', marginBottom:5 }}>
      ...
      <div style={{ flex:'0 0 45%', display:'flex', flexDirection:'column', borderRight:'1px solid #e0ddd7', background:'#fff', overflow:'hidden' }}>
        <div style={{ padding:'10px 16px', borderBottom:'1px solid #e8e5e0', fontSize:11, fontWeight:600, color:BRAND.charcoal, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
        ...
        <div style={{ flex:1, overflow:'auto', background:colView==='preview'?'#f0ede8':'#fff' }}>
          ...
          {selected && colView==='preview' && (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:16, padding:24 }}>
              <div style={{ width:'100%', maxWidth:380, background:'#f7f5f1', border:'1px solid #e0ddd7', borderRadius:8, overflow:'hidden' }}>
                ...
                <div style={{ padding:14, display:'flex', gap:14, alignItems:'flex-start' }}>
                  <div style={{ width:88, height:100, background:'#fff', border:'1px solid #e0ddd7', borderRadius:4, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, overflow:'hidden' }}>
                    {selected.previewUrl?<img src={selected.previewUrl} alt="" style={{ maxWidth:'100%', maxHeight:'100%', objectFit:'contain' }}/>:<div style={{ fontSize:28, opacity:0.2 }}>⬡</div>}
                  </div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:BRAND.charcoal, marginBottom:5, wordBreak:'break-all' }}>{selected.name}</div>
                    <div style={{ fontSize:10, color:'#bbb' }}>{selected.fileType?.includes('pdf')?'PDF':selected.fileType?.split('/')[1]?.toUpperCase()||'File'}</div>
              ...
              <button onClick={()=>setColView('phases')} style={{ background:BRAND.red, color:'#fff', border:'none', borderRadius:8, padding:'11px 32px', fontSize:13, fontWeight:600, cursor:'pointer' }}>Select QA phases →</button>
            </div>
          )}
          {selected && colView==='phases' && (
            <div style={{ padding:'16px 20px' }}>
              ...
              {PHASES.map(([num,label])=>{
                ...
                return (
                  <div key={num} onClick={()=>setSelectedPhases(prev=>{const n=new Set(prev);n.has(num)?n.delete(num):n.add(num);return n})} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:on?'#fff':'#fafaf8', border:`1.5px solid ${on?BRAND.red:'#e8e5e0'}`, borderRadius:8, cursor:'pointer', marginBottom:6 }}>
                    <div style={{ width:22, height:22, borderRadius:6, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:on?BRAND.red:'transparent', border:on?'none':'2px solid #ccc' }}>
                      {on && <svg width="12" height="10" viewBox="0 0 12 10"><polyline points="1,5 4.5,8.5 11,1" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
              ...
              <div style={{ borderTop:'1px solid #e8e5e0', paddingTop:12, marginTop:4, marginBottom:14 }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {savedPresets.map(p=>{
                    ...
                    return <div key={p.name} onClick={()=>setSelectedPhases(new Set(p.phases))} style={{ fontSize:11, padding:'5px 12px', background:'#fff', border:`1.5px solid ${active?BRAND.red:'#e0ddd7'}`, borderRadius:20, color:active?BRAND.red:'#666', cursor:'pointer', fontWeight:active?500:400 }}>{p.name}</div>
                  })}
                  <div onClick={()=>{const name=prompt('Preset name?');if(name)setSavedPresets(prev=>[...prev,{name,phases:[...selectedPhases].sort((a,b)=>a-b)}])}} style={{ fontSize:11, padding:'5px 12px', background:'#fff', border:'1.5px dashed #ddd', borderRadius:20, color:'#bbb', cursor:'pointer' }}>+ Save current</div>
                </div>
              </div>
              <button onClick={()=>selectedPhases.size>0&&runQA(selected,selectedPhases)} style={{ width:'100%', background:selectedPhases.size>0?BRAND.red:'#ccc', color:'#fff', border:'none', borderRadius:8, padding:11, fontSize:13, fontWeight:600, cursor:selectedPhases.size>0?'pointer':'default' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0, background:'#fff' }}>
        <div style={{ padding:'8px 14px', borderBottom:'1px solid #eee', background:'#fafaf8', display:'flex', alignItems:'center', gap:6 }}>
          ...
          <span style={{ fontSize:11, fontWeight:600, color:BRAND.charcoal }}>{selected?.result?`Chat — ${selected.result.drawingRef}`:'Chat'}</span>
        </div>
        <div style={{ flex:1, overflowY:'auto', padding:'10px 12px' }}>
          {msgs.length===0 && <div style={{ fontSize:11, color:'#ccc', textAlign:'center', marginTop:20 }}>{selected?.result?'Ask anything about this drawing':'Select a drawing to begin chatting'}</div>}
          {msgs.map((m,i)=>(
            <div key={i} style={{ display:'flex', justifyContent:m.role==='user'?'flex-end':'flex-start', marginBottom:7 }}>
              <div style={{ maxWidth:'85%', fontSize:12, lineHeight:1.6, padding:'6px 11px', borderRadius:m.role==='user'?'13px 13px 3px 13px':'13px 13px 13px 3px', background:m.role==='user'?BRAND.charcoal:'#f0ede8', color:m.role==='user'?BRAND.chalk:BRAND.charcoal }}>{m.content}</div>
            </div>
          ))}
          {chatLoading && <div style={{ display:'flex', gap:4, padding:'4px 2px' }}>{[0,1,2].map(i=><div key={i} style={{ width:6, height:6, borderRadius:'50%', background:'#ccc', animation:`dp 1.2s ${i*0.2}s infinite` }}/>)}</div>}
          <div ref={chatBottomRef}/>
        </div>
        <div style={{ padding:'8px 10px', borderTop:'1px solid #eee', display:'flex', gap:6 }}>
          <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&sendChat()} placeholder={selected?.result?'Ask about this drawing…':'Select a drawing first…'} disabled={!selected?.result} style={{ flex:1, padding:'7px 11px', borderRadius:7, border:'1.5px solid #ddd', fontSize:12, outline:'none', fontFamily:'inherit', background:selected?.result?'#fff':'#f7f7f7' }}/>
